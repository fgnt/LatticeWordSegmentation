#!/bin/bash
################################################################################################################################################
### Call: StartSim_openfst_lattice_increase_fixed.bash FileListPath KnownN UnkN NumIter AmScale SwitchIterIdx NewKnownN NewUnkN NewLMNumIters ##
### e.g.: ./StartSim_openfst_lattice_increase_fixed.bash WSJCAM0_Lattice/WSJCAM0_Phoneme_Lattice.10.txt 1 2 200 1 35 2 6 0                    ##
###                                                                                                                                           ##
### Runs segmentation on openfst lattice input with word LM order KnownN, character LM  order UnkN for NumIter iterations. Will switch        ##
### language model orders at SwitchIterIdx to new word LM order NewKnownN and new character LM order NewUnkN. After switching the LMs         ##
### will be retrained for NewLMNumIters on the current segmentation result. Segmentation is done by Gibbs sampling for 150 iterations,        ##
### then by Viterby decoding for another 25 iterations. From iteration 175 on the character language model is deactivated by setting d        ##
### and Theta to zero for the 0-gram word language model. The language model is always estimated by Gibbs sampling. Estimation of the         ##
### word length distribution and correction factors is not done                                                                               ##
###                                                                                                                                           ##
### Segmentation results are saved in Results/${Path}_increase_fixed/${FileList}.                                                             ##
###                                                                                                                                           ##
