#!/bin/bash
#########################################################################################################################
### Call: StartSim_openfst_lattice_fixed_lm_pretain.bash FileListPath KnownN UnkN NumIter AmScale InitLmNumIterations  ##
### e.g.: ./StartSim_openfst_lattice_fixed_lm_pretrain.bash WSJCAM0_Lattice/WSJCAM0_Phoneme_Lattice.10.txt 1 2 200 1 1 ##
###                                                                                                                    ##
### Runs segmentation on openfst lattice input with word LM order KnownN, character LM  order UnkN                     ##
### for NumIter iterations. Language model initialization is done with the reference transcriptions                    ##
### Segmentation is done by Gibbs sampling for 150 iterations, then by Viterby decoding for another 25 iterations.     ##
### From iteration 175 on the character language model is deactivated by setting d and Theta to zero for the 0-gram    ##
### word language model. The language model is always estimated by Gibbs sampling. Estimation of the word length       ##
### distribution and correction factors is not done.                                                                   ##
###                                                                                                                    ##
...
InitLmNumIterations="-InitLmNumIterations ${6}"                                           # Number of iterations for language model initialization (Parameter: -InitLmNumIterations NumIterations (0))
...
InitLM="-InitLM ${FileListPath}.ref"                                                      # initialize language model from initialization fsts (Parameter: -InitLM InitTranscriptionFilename ())
